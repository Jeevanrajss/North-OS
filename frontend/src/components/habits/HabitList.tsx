...
export function HabitList({ habits, loading, onCreate }: Props) {
  ...
  return (
    <div className="card">
      <div className="card-title">Habit List</div>
      {loading ? (
      ...
      <div className="mt-3 pt-3 border-t border-ink-800">
        <button
          ...
          className="flex items-center gap-1 text-[11px] text-ink-500 hover:text-ink-300 transition-colors w-full"
        >
          {archivedOpen ? (
        ...
        {archivedOpen && (
          <div className="mt-2">
            {archivedLoading ? (
              <div className="text-xs text-ink-600 text-center py-2">Loading…</div>
            ) : archivedHabits.length === 0 ? (
              <div className="text-xs text-ink-600 text-center py-2">No archived habits.</div>
            ) : (
              <ul className="space-y-0.5">
  ...
  return (
    <li className="flex items-center gap-2 rounded-md px-1.5 py-1.5 opacity-50 hover:opacity-75 transition-opacity">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[10px] text-ink-600 truncate">archived {archivedDate}</div>
      </div>
      <button
        type="button"
...
function HabitListRow({ habit, streak, onSave, onArchive }: RowProps) {
  ...
  return (
    <li className="group flex items-center gap-2 rounded-md px-1.5 py-1.5 hover:bg-ink-950">
      <Link
        to={`/habits/${habit.id}`}
        className="flex items-center gap-2 flex-1 min-w-0"
        title={`View ${habit.name} details`}
      >
        <span className="w-6 text-center text-base leading-none">{habit.emoji}</span>
        <div className="flex-1 min-w-0">
          <div className="text-sm text-ink-100 hover:text-accent truncate transition-colors">
            {habit.name}
          </div>
          <div className="text-[10px] text-ink-500 truncate">{scheduleLabel}</div>
        </div>
      </Link>
      {streak > 0 && (
        <span
          className="inline-flex items-center gap-0.5 text-[11px] text-amber-400 tabular-nums"
      ...
      <div
        className={cn(
          ...
          confirmArchive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 focus-within:opacity-100',
        )}
      >
        {confirmArchive ? (
          <>
