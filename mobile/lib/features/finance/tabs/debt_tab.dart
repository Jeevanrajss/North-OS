...
class _DebtTabState extends ConsumerState<DebtTab> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final results = await Future.wait([dio.get('/finance/debt'), dio.get('/finance/debt/summary')]);
      setState(() {
                  ...
                  Expanded(
                    child: Column(
                      children: [
                        Text(
                          fmt.format(totalOut),
                          style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: NorthColors.amber),
                        ),
                        Text('Outstanding', style: TextStyle(fontSize: 11, color: NorthColors.fg5)),
                      ],
                    ),
                  ),
                  Expanded(
                    child: Column(
                      children: [
                        ...
                        Text('EMI/month', style: TextStyle(fontSize: 11, color: NorthColors.fg5)),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _debtCard(Debt d, NumberFormat fmt) {
    final rateColor = d.interestRate > 15
        ? NorthColors.red
        : d.interestRate > 5
        ? NorthColors.amber
        : NorthColors.green;
    ...
    final dueColor = (daysUntilDue != null && daysUntilDue <= 3)
        ? NorthColors.red
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              fmt.format(d.outstanding),
              style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: NorthColors.fg1),
            ),
            ...
            Row(
              children: [
                Text('EMI: ${fmt.format(d.emiAmount)}/mo', style: TextStyle(fontSize: 12, color: NorthColors.fg4)),
                const Spacer(),
